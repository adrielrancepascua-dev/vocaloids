...
function CharacterTabsBase({ characterData }: CharacterTabsProps) {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center">
      <div className="relative flex flex-wrap items-center justify-center gap-x-12 gap-y-6 border-b border-white/15 pb-4 w-full">
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className="relative pb-2 text-xl md:text-2xl font-bold uppercase tracking-[0.16em] transition-colors duration-200"
              style={{
        ...
        >
          {activeTab === 'autobiography' && (
            <div className="flex flex-col items-center gap-6">
              <p className="text-xl md:text-2xl tracking-[0.2em] font-medium uppercase font-inter-tight" style={{ color: characterData.colors.primary }}>
                {characterData.loreMetric}
              </p>
              <p className="text-2xl md:text-3xl leading-relaxed text-zinc-200 max-w-3xl font-light">{characterData.bio}</p>
            </div>
          )}

          {activeTab === 'core-stats' && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-3xl">
              <div className="flex flex-col items-center p-6 border border-white/10 rounded-2xl bg-black/20 backdrop-blur-sm">
                <span className="text-sm uppercase tracking-[0.3em] text-zinc-500 mb-2">Age</span>
                <span className="text-4xl font-bold font-inter-tight" style={{ color: characterData.colors.primary }}>
                  {characterData.stats.age}
                </span>
              </div>
              <div className="flex flex-col items-center p-6 border border-white/10 rounded-2xl bg-black/20 backdrop-blur-sm">
                <span className="text-sm uppercase tracking-[0.3em] text-zinc-500 mb-2">Height</span>
                <span className="text-4xl font-bold font-inter-tight" style={{ color: characterData.colors.primary }}>
                  {characterData.stats.height}
                </span>
              </div>
              <div className="flex flex-col items-center p-6 border border-white/10 rounded-2xl bg-black/20 backdrop-blur-sm">
                <span className="text-sm uppercase tracking-[0.3em] text-zinc-500 mb-2">Weight</span>
                <span className="text-4xl font-bold font-inter-tight" style={{ color: characterData.colors.primary }}>
                  {characterData.stats.weight}
                </span>
              </div>
              <div className="md:col-span-3 flex flex-col items-center p-6 border border-white/10 rounded-2xl bg-black/20 backdrop-blur-sm mt-4">
                <span className="text-sm uppercase tracking-[0.3em] text-zinc-500 mb-2">System Status</span>
                <span className="text-2xl font-bold font-inter-tight tracking-wider" style={{ color: characterData.colors.primary }}>
          ...
          {activeTab === 'song-stats' && (
            <ul data-song-list="true" className="space-y-8 w-full max-w-2xl text-left">
