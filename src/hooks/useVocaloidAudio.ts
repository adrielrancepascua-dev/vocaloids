...
const TRACKS: Record<VocaloidTrackName, TrackConfig> = Object.fromEntries(
  vocaloidData.map((v) => [getEffectsForTheme(v.theme).defaultTrack, { src: v.audioSrc }]),
) as Record<VocaloidTrackName, TrackConfig>;
...
export function useVocaloidAudio(): UseVocaloidAudioReturn {
  ...
  const setMasterVolume = useCallback((volume: number) => {
    ...
    try {
      ...
      masterGain.gain.linearRampToValueAtTime(volume, now + 0.3);
    } catch {}
  }, []);
